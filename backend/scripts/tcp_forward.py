...
The CMC CW1000 simulator only allows a fixed IP dropdown (127.0.0.1 or a
hardcoded LAN IP), so it cannot directly reach Railway. Run this script on
the Windows PC that runs the simulator; it accepts on localhost and pipes
bytes to the Railway TCP proxy target.

Usage:
    python tcp_forward.py
    # then in the simulator: IPserver=127.0.0.1, Port=15001, press OPEN

Change REMOTE_HOST / REMOTE_PORT if Railway assigns a different proxy target.
"""
...
def main() -> int:
    ...
    print("[*] point the CMC simulator at 127.0.0.1:15001, then press OPEN")
    try:
        while True:
