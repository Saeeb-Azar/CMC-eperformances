            ...
            {visible.length === 0 ? (
              ...
              visible.map((event, idx) => {
                ...
                return (
                  <tr key={event.id}>
                    <td>
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${sev.bg} ${sev.color}`}>
                        {icon}
                      </div>
                    </td>
                    <td>
                      <span className={isFirst ? 'cell-primary' : ''}>{event.message}</span>
                    </td>
                    <td><span className="cell-mono">{event.technicalCode}</span></td>
                    <td><span className="cell-mono">{event.referenceId || <span className="cell-empty">—</span>}</span></td>
                    <td className="!text-right"><span className="cell-muted tabular-nums">{event.timestamp}</span></td>
                  </tr>
                );
              })
            )}
